...
import (
	...
	"github.com/longportapp/whaleapi-sdk/go/trade/jsontypes"
...
func (c *core) Subscribe(ctx context.Context, topics []string) (*SubResponse, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *core) resubscribe(ctx context.Context) error {
	...
	if len(res.Fail) > 0 {
		log.Errorf("resubscribe some topics failed %v", res.Fail)
	}
	return nil
}
